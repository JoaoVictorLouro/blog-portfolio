(function () {
  ...
  const COMMENTS_TOKENS = {
    ...
    light: {
      ...
      surfaceHigh: '#e6e8ed',
      onSurface: '#0d0e10',
      onSurfaceVariant: '#3a4249',
      outline: '#4e575e',
      outlineVariant: '#9aa1a8',
      charcoal: '#0d0e10',
      neonTeal: '#00f2ff',
      lanternAmber: '#ffb800',
      cyberMagenta: '#ff00c8',
  ...
  function commentsTokenCss(mode, bg) {
    ...
    return [
      ...
      `--np-comment-link:${mode === 'light' ? t.onSurface : t.neonTeal};`,
      `--np-comment-ghost:${mode === 'light' ? t.onSurface : t.lanternAmber};`,
      '}',
    ].join('');
  }
  ...
  function isNavLinkActive(linkPath, currentPath) {
    ...
    if (linkPath.endsWith('/articles/')) {
      const localeRoot = linkPath.replace(/articles\/$/, '');
      if (currentPath.startsWith(`${localeRoot}tag/`)) {
        return true;
      }
    }
    return false;
