...
|------|----------|---------|
| `content/themes/` | yes | Themes (Source vendored under `content/themes/source`) |
...
| `.env` | **no** | Local secrets — use `.env.example` |
...
task lint        # gscan on content/themes/source
...
- Commit `.env`, `data/`, or `node_modules/`
...
- [`.cursorignore`](.cursorignore) excludes `.env` / `.env.*` (keeps `.env.example`) and `data/`
